...
export function RunsView() {
	...
	const columns = useMemo<ColumnDef<Run>[]>(() => {
			...
			},
			{
				...
				cell: ({ row }) => (
					<div className="flex items-center min-w-0 text-[13px] text-white/60">
						<div className="inline-flex items-center px-2 py-0.5 rounded-md bg-white/4 border border-white/5 truncate max-w-full font-mono text-[11.5px]">
		...
		if (!authEnabled) {
			return cols.filter((c: ColumnDef<Run>) => (c as { accessorKey?: string }).accessorKey !== 'namespace');
		}
	...
	return (
		<div className="flex flex-col h-full bg-zinc-950">
			...
			<div className="flex flex-col gap-6 px-6 lg:px-8 py-6 border-b border-white/5 bg-neutral-950">
				<div>
					<div className="flex items-center gap-3 mb-2">
						<h1 className="text-2xl font-bold tracking-tight text-white">
							Live Feed
						</h1>
						<div className="flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/5 px-2.5 py-1 backdrop-blur-md">
				...
				<div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
					<div className="flex-none w-44 flex flex-col p-4 rounded-xl bg-white/2 border border-white/4 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.02)]">
					...
					<div className="flex-none w-40 flex flex-col p-4 rounded-xl bg-blue-500/3 border border-blue-500/10 shadow-[inset_0_1px_0_0_rgba(59,130,246,0.02)] relative overflow-hidden">
					...
					<div className="flex-none w-48 flex flex-col p-4 rounded-xl bg-red-500/2 border border-red-500/5 shadow-[inset_0_1px_0_0_rgba(239,68,68,0.01)] relative overflow-hidden">
					...
					<div className="flex-none flex items-center justify-between gap-4">
						<div className="relative w-full max-w-sm">
							...
							<input
								...
								className="w-full bg-zinc-900/80 border border-white/8 rounded-full h-10 pl-10 pr-4 text-[13px] text-white placeholder:text-white/30 focus:outline-none focus:border-white/20 transition-colors"
							/>
						</div>
						...
						<DropdownMenu>
							<DropdownMenuTrigger
								render={
									<Button
										variant="outline"
										className="gap-2 h-10 rounded-full px-4 border-white/8 bg-zinc-900/80 hover:bg-white/4"
									/>
								}
							>
								<HugeiconsIcon
						...
						{error ? (
							...
							<div className="w-full">
								<div className="flex items-center px-4 h-11 border-b border-transparent">
									<div className="w-24 h-3 bg-white/3 rounded-sm animate-pulse" />
									<div className="w-32 h-3 bg-white/3 rounded-sm animate-pulse ml-auto" />
								</div>
